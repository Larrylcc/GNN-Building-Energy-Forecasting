...
OUTPUT_DIR = WORKSPACE_ROOT / "STGNN" / "stgnn_gcn_gru_auto-rolling_outputs"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train an STGNN + GCN + GRU model.")
    parser.add_argument(
        "--smoke-test",
        action="store_true",
...
def save_training_history(training_history: pd.DataFrame, output_path: Path = TRAINING_HISTORY_PATH) -> Path:
...
def save_graph_artifacts(
    ...
    topk_edges_path: Path = TOPK_EDGES_PATH,
) -> tuple[Path, Path]:
    model.eval()
    with torch.no_grad():
...
def build_smoke_frames() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, list[str]]:
    columns = pd.read_csv(TRAIN_DATA_PATH, nrows=0).columns.tolist()
...
def main(
    ...
    training_history_path = save_training_history(training_history=training_history, output_path=TRAINING_HISTORY_PATH)
