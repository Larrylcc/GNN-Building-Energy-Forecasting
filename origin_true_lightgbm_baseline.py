...
try:
    ...
    raise ImportError(
        "lightgbm is required for this baseline. Install it in the notebook kernel with `%pip install lightgbm`."
    ) from exc
...
OUTPUT_DIR = Path(r"F:\Desktop\Final\workspace\origin_true_lightgbm_baseline_outputs")
...
def build_lgbm_params(seed: int = SEED) -> dict:
    return {
...
def train_lightgbm_cv(
    ...
    params = build_lgbm_params(seed=seed)
...
def predict_test_in_chunks(
    ...
    for start in range(0, test_df.shape[0], chunk_size):
        ...
        for model in models:
            chunk_pred += model.predict(chunk, num_iteration=model.best_iteration_) / len(models)

        predictions_log[start:stop] = chunk_pred
        print(f"Predicted rows {start:,} to {stop:,}")

        del chunk, chunk_pred
        gc.collect()
...
def save_metrics(fold_scores: list[float], overall_rmse: float, output_dir: Path) -> Path:
    ...
    metrics_path = output_dir / "lightgbm_cv_metrics.csv"
...
def save_submission(sample_submission: pd.DataFrame, predictions: np.ndarray, output_dir: Path) -> Path:
    ...
    submission_path = output_dir / "lightgbm_submission.csv"
...
def main(data_root: Path = DATA_ROOT, output_dir: Path = OUTPUT_DIR) -> dict:
    ...
    training_results = train_lightgbm_cv(train_df=train_df)
    ...
    summary_path = output_dir / "lightgbm_run_summary.json"
